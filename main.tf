...
module "oke" {
  ...
  # Bastion variables
  create_bastion           = var.create_cluster ? var.create_operator_and_bastion : false
  ...
  # Operator variables
  create_operator                          = var.create_cluster ? var.create_operator_and_bastion : false
  ...
  # Network variables
  create_vcn               = var.create_cluster ? var.create_vcn : false
  lockdown_default_seclist = true           # *true/false
  vcn_id                   = var.create_cluster ? var.vcn_id : null    # Ignored if create_vcn = true
  ...
  cluster_id                        = var.create_cluster ? null : var.cluster_id
  ...
  worker_pools = yamldecode(var.create_cluster ? yamlencode(local.workers) : yamlencode({}))

  output_detail = true
}

output "bastion" {
  value = "%{if alltrue([var.create_operator_and_bastion, var.create_cluster])}${module.oke.bastion_public_ip}%{else}bastion host not created.%{endif}"
}

output "operator" {
  value = "%{if alltrue([var.create_operator_and_bastion, var.create_cluster])}${module.oke.operator_private_ip}%{else}operator host not created.%{endif}"
}

output "ssh_to_operator" {
  value = "%{if alltrue([var.create_operator_and_bastion, var.create_cluster])}${module.oke.ssh_to_operator}%{else}bastion and operator hosts not created.%{endif}"
}
